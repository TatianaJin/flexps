...
ASPModel::ASPModel(uint32_t model_id, std::unique_ptr<AbstractStorage>&& storage_ptr,
  ...
  LOG(INFO) << "[ASPModel] Version, Timestamp: " << 0 << ","
...
void ASPModel::Clock(Message& msg) {
  ...
  if (dump_interval_ > 0 && updated_min_clock > 0 && updated_min_clock % dump_interval_ == 0) {
    LOG(INFO) << "[ASPModel] Version, Timestamp: " << updated_min_clock << ","
              << std::chrono::duration_cast<std::chrono::milliseconds>(
                     std::chrono::steady_clock::now().time_since_epoch())
                     .count();
...
void ASPModel::Dump(int server_id, const std::string& path) {
  storage_->WriteTo(path + "MODEL_v" + std::to_string(progress_tracker_.GetMinClock()) + "_part" +
                    std::to_string(server_id));
}

void ASPModel::Load(const std::string& file_name) { storage_->LoadFrom(file_name); }
