...
DEFINE_int32(report_interval, 0, "training loss report interval");
...
using DataObj = lib::TypedLabeledSample<float, float>;
...
void Run() {
  ...
  auto table_id = engine.CreateTable<float>(model_type, storage_type, num_params, FLAGS_kStaleness, 1);
  uint32_t g_table_id = -1;  // full gradient table
  if (FLAGS_optimizer == "svrg") {
    // BSP model, reset every iter
    g_table_id = engine.CreateTable<float>(ModelType::BSPResetAdd, storage_type, num_params, 0, 1);
  }
  engine.Barrier();
  ...
  if (FLAGS_trainer == "logistic") {
    objective_ptr = std::make_shared<SigmoidObjective>(num_params);
  } else if (FLAGS_trainer == "linear") {
    objective_ptr = std::make_shared<LinearRegressionObjective>(num_params);
  } else {
  ...
  if (FLAGS_optimizer == "sgd") {
    optimizer_ptr = std::make_shared<SGDOptimizer>(objective_ptr, FLAGS_report_interval);
  } else if (FLAGS_optimizer == "svrg") {
    optimizer_ptr = std::make_shared<SVRGOptimizer>(objective_ptr, FLAGS_report_interval);
    conf.num_epoches = 5;
